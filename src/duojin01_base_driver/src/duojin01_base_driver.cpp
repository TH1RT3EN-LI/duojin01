...
private:
    ...
    void start_serial_receive()
    {
        if (!serial_)
        {
            RCLCPP_ERROR(this->get_logger(), "SerialPort not created");
            return;
        }
        try
        {
    ...
    void on_serial_data(std::vector<uint8_t> &data, size_t n)
    {
        ...
        try
        {
            ...
            {
                serial_->async_receive(
                    [this](std::vector<uint8_t> &d, const size_t bytes)
                    {
                        this->on_serial_data(d, bytes);
                    });
            }
        }
        catch (const std::exception &e)
        {
            RCLCPP_ERROR(this->get_logger(), "async_receive re-register failed: %s", e.what());
        }
    }
    /**
     * @brief 获取一帧有效数据
...
private:
    void try_open_serial_port()
    {
        ...
        {
            // Throttle to avoid log spam when the port isn't present yet (typical in sim bringup).
            RCLCPP_WARN_THROTTLE(
                this->get_logger(),
                *this->get_clock(),
                2000,
