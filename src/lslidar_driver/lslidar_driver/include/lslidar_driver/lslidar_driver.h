...
private:    
    ...
    void data_processing_2(unsigned char *packet_bytes,int len);
	    ...
	    int getScan(std::vector<ScanPoint> &points, rclcpp::Time &scan_time, float &scan_duration, int &count);
    ...
    boost::condition_variable pubscan_cond_;
	    ...
	    int UDP_PORT_NUMBER;
	    int count_num;
	    int count_num_bak_ = 0;  // snapshot synced with scan_points_bak_ to avoid cross-thread mismatches
	    int package_points;
    int data_bits_start;
    ...
    int idx = 0;
    int link_time = 0;
    int expected_scan_count_ = 0;  // Lock LaserScan size to avoid per-scan length jitter
    int fixed_scan_count_ = 0;     // Optional override for fixed LaserScan size
    int scan_count_warmup_ = 5;    // Number of scans to observe before locking size
    int scan_count_samples_ = 0;
    int max_scan_count_ = 0;
    bool scan_count_locked_ = false;
